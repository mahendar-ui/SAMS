...
export class AsideLeftComponent implements OnInit, AfterViewInit {
  ...
  isCurentUserAccess(item){
    let accessRoles = item.roleAccess;
    ...
    if(accessRoles){
      ...
      for(let i=0;i<=accessRoles.length;i++){
        // if(accessRoles.indexOf(currentUserRoles[i]) > -1){
        //   findRole = true;
        // }
        if(accessRoles.find(role => role.type === currentUserRoles)){
          findRole = true;
        }
      }
      if(findRole){
